...
class FindConfigPathTests(unittest.TestCase):
    ...
    def test_permission_error_candidate_is_skipped(self) -> None:
        denied = Path("C:/denied/config.json")
        valid = Path("C:/valid/config.json")
        logs: list[str] = []
        ...
        with (
            ...
            self.assertFalse(backup._path_exists(denied))
            self.assertTrue(backup._path_exists(valid))

        self.assertTrue(any("unable to access" in message for message in logs))
...
class ResolveCommandTests(unittest.TestCase):
    ...
    def test_run_command_uses_resolved_executable_path(self) -> None:
        ...
        with (
            patch.object(backup, "_resolve_command", return_value=resolved),
            patch.object(
                backup.subprocess,
                "run",
            ...
            self.assertTrue(backup._run_command_safely(["tool", "status"]))

        self.assertEqual(run_mock.call_args.args[0], [resolved, "status"])

    def test_command_output_decode_replaces_invalid_codepage_bytes(self) -> None:
        decoded = backup._decode_command_output(b"message:\xff")
        self.assertTrue(decoded.startswith("message:"))
