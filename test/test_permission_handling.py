...
class TestFilePermissionHandling(unittest.TestCase):
    ...
    def test_fix_missing_read_permission(self):
        """测试：自动修复缺失的读权限"""
        if os.name == "nt":
            self.skipTest("Windows chmod does not model NTFS read ACLs")
        with tempfile.NamedTemporaryFile(mode='w', delete=False) as f:
            f.write("test content")
            temp_path = Path(f.name)
    ...
    def test_fix_missing_write_permission(self):
        """测试：自动修复缺失的写权限"""
        if os.name == "nt":
            self.skipTest("Windows chmod does not model NTFS write ACLs")
        with tempfile.NamedTemporaryFile(mode='w', delete=False) as f:
            f.write("test content")
            temp_path = Path(f.name)
...
class TestDirectoryPermissionHandling(unittest.TestCase):
    ...
    def test_fix_directory_write_permission(self):
        """测试：修复目录写权限"""
        if os.name == "nt":
            self.skipTest("Windows chmod does not model NTFS directory ACLs")
        with tempfile.TemporaryDirectory() as temp_dir:
            dir_path = Path(temp_dir)
